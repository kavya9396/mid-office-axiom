...
const Decision = () => {
  ...
  sessionStorage.getItem("caseDecision") || ""
    ...
    const masterData = useAppSelector((state: RootState) => (state as unknown as Record<string, unknown>).masterData);
...
const masterDataObj = masterData as unknown as Record<string, unknown> | undefined;
const miscFromStore = (masterDataObj && (masterDataObj['data'] as Record<string, unknown> | undefined))
  ? ((masterDataObj['data'] as Record<string, unknown>)['data'] as Record<string, unknown> | undefined)?.['misc']
  : undefined;
const miscData =
  miscFromStore || JSON.parse(sessionStorage.getItem("masterData") || "{}")?.data?.misc || [];
...
console.log('decisionOptions',decisionOptions)
  return (
    <>
      <Box sx={{ p: 1 }}>
        <CustomAccordion title={"Decision"} defaultExpanded>
          <Box
            sx={{
              borderRadius: "6px",
