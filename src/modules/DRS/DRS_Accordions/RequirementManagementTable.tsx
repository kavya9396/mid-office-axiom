...
const INITIAL_FORM_STATE: CVTRequirementRow = {
    team: "",
    profile: "",
    category: "",
    subCategory: "",
...
const RequirementManagementTable = ({ requirements }: RequirementManagementTableProps) => {
    ...
    const [requirementForm, setRequirementForm] = useState<CVTRequirementRow>(INITIAL_FORM_STATE);
    const [submittedRequirements, setSubmittedRequirements] = useState<CVTRequirementRow[]>([]);

    const requirementRows: CVTRequirementRow[] = [
        ...(finalRequirements ?? []),
    ...
    const handleSubmit = () => {
        setSubmittedRequirements((prev) => [...prev, { ...requirementForm }]);
        setRequirementForm(INITIAL_FORM_STATE);
    ...
    return (
        <>
            ...
            <CustomDialog
                ...
                maxWidth="md"
                title={
                    <Typography sx={{ ...modalTitleStyles }}>add requirement</Typography>
                }
                        ...
                        >
                            {row.map((field) => (
                                <Box key={field.name}>
                                    <CustomSelect
                                        label={field.label}
                                        value={requirementForm[field.name as keyof CVTRequirementRow] as string}
