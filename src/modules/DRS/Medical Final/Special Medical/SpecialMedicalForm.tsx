...
type SpecialMedicalMasterOptions = Record<
  SpecialMedicalMasterKey,
...
const buildMasterOptions = (
  ...
  const byType = (type: SpecialMedicalMasterKey) =>
    toOptionList(
      miscMasters.filter(
        (option) =>
          String(option.type ?? "").trim().toUpperCase() === type
  ...
  return {
    MEDICAL_TYPE: byType("MEDICAL_TYPE"),
    FINDINGS: byType("FINDINGS"),
    SM_FINDINGS: byType("SM_FINDINGS"),
    AXIS: byType("AXIS"),
    FINDINGS_RESULT: byType("FINDINGS_RESULT"),
    METS: byType("METS"),
  };
...
const resolveMasterFieldValue = (
  field: SpecialMedicalFieldConfig,
  value: string,
  masterOptions: SpecialMedicalMasterOptions
) => {
  if (!field.masterKey) {
    return value;
  }
...
useImperativeHandle(
  ...
  (): SpecialMedicalFormHandle => ({
    ...
    getFormValues: () =>
      subsectionFields.reduce<Record<string, string>>(
        (values, field) => {
          values[field.id] = resolveMasterFieldValue(
            field,
            formValues[field.id] ?? "",
            masterOptions
          );
  ...
  [formValues, masterOptions, subsectionFields]
);
  ...
  return (
    ...
    >
      {subsectionFields.length === 0 ? (
        ...
        >
          {subsectionFields.map((field) => {
            const value = resolveMasterFieldValue(
              field,
              formValues[field.id] ?? "",
              masterOptions
            );
            ...
            const options = field.masterKey
              ? masterOptions[field.masterKey] ?? []
              : [];
            const isDropdown = field.type === "dropdown" || Boolean(field.masterKey);
